...
export default function Home() {
  ...
  const openDashboardModal = (e?: React.MouseEvent) => {
  ...
  useEffect(() => {
    const onKey = (ev: KeyboardEvent) => {
      if (ev.key === 'Escape') closeDashboardModal();
    };
    if (showDashboardModal) window.addEventListener('keydown', onKey);
  ...
  useEffect(() => {
    let animationFrameId = 0 as unknown as number;
    let mouseX = cursor.x;
    let mouseY = cursor.y;

    const updateCursor = () => setCursor({ x: mouseX, y: mouseY });
    ...
    // intentionally empty deps to attach once
    ...
    <>
      ...
      >
        ...
        Hey developer!, View your <span>
          <a href="#" onClick={openDashboardModal} className="dashboard-link">
            dashboard
      ...
      <div className="tagline-section">
        <style>{`
          ...
          @media (max-width: 900px) {
            .main-tagline { font-size: 1rem; max-width: 95vw; margin-left: 2vw; }
            .main-tagline-btn { font-size: 1rem; padding: 0.6rem 1.5rem; }
          }
        `}</style>
      ...
      <div style={{ position: "absolute", top: "34%", right: "3vw", transform: "translateY(-50%)", zIndex: 10, pointerEvents: "none" }}>
        <Image
          src="/landguy.png"
          alt="Landing Guy"
          width={220}
          height={220}
          style={{ maxWidth: "18vw", height: "auto", minWidth: "120px", opacity: 0.97, userSelect: "none" }}
          draggable={false}
          priority
        />
      </div>

      <div style={{ width: "100vw", display: "flex", justifyContent: "center", alignItems: "center", marginTop: "15.5rem", marginBottom: "2.5rem", zIndex: 1, position: "relative", flexDirection: "column" }}>
        <style>{`
          .bento-grid {
            display: grid;
          ...
          .bento-title { font-size: 1.08rem; color: #fff; font-weight: 600; margin-bottom: 0.6rem; letter-spacing: 0.01em; }
          .bento-value { font-size: 2.1rem; font-weight: 700; color: #fff; letter-spacing: 0.01em; line-height: 1.1; }
          .bento-sub { font-size: 0.98rem; color: #d1d5db; margin-top: 0.2rem; font-weight: 400; letter-spacing: 0.01em; }
          .bento-card2 {
            background: rgba(255,255,255,0.07);
            border-radius: 1.2rem;
            border: 1.5px solid rgba(255,255,255,0.13);
          ...
          @media (max-width: 900px) {
            .bento-grid { grid-template-columns: 1fr 1fr; gap: 1.2rem; width: 96vw; }
            .bento-card2 { width: 96vw; max-width: 100vw; padding: 1.2rem 0.7rem 1.1rem 0.7rem; min-height: 90px; }
            .bento-card { padding: 1.2rem 0.7rem 1.1rem 0.7rem; min-height: 90px; }
        ...
        <div className="bento-grid">
          ...
          <div className="bento-card" ref={submissionsRef}>
            ...
            <div className="bento-value">{submissionsInView ? <CountUp end={1542} duration={2} separator="," /> : "0"}+</div>
          ...
          <div className="bento-card" ref={hackersRef}>
            ...
            <div className="bento-value">{hackersInView ? <CountUp end={4800} duration={2} separator="," /> : "0"}+</div>
      ...
      <LogoLoop logos={techLogos} speed={120} direction="left" logoHeight={68} gap={120} scaleOnHover ariaLabel="Technology partners" />

      <div className="mt-15 mb-50">
        <LogoLoop logos={techLogos} speed={120} direction="right" logoHeight={68} gap={120} scaleOnHover ariaLabel="Technology partners" />
      </div>

      <footer className="w-full border-t border-white/20 text-white backdrop-blur-md bg-transparent cursor-none">
        <style>{`
          .footer-spacemono { font-family: 'Space Mono', monospace !important; }
          .footer-josefin { font-family: 'Josefin Sans', sans-serif !important; letter-spacing: 0.04em; }
        `}</style>
        ...
        <div className="max-w-7xl mx-auto px-6 py-10 grid grid-cols-1 md:grid-cols-4 gap-8 footer-spacemono cursor-none">
          <div className="col-span-1">
            ...
            <p className="text-sm text-gray-400 mt-2">AI-assisted judging for fairer, faster hackathons.</p>

            <div className="mt-4">
              <p className="text-sm text-gray-300 mb-2">Get updates from the <span className="footer-josefin"> DeVerify </span> team:</p>
              <div className="flex items-center bg-white/5 border border-white/30 rounded-md overflow-hidden w-full max-w-sm">
                <input type="email" placeholder="Email" className="flex-1 bg-transparent text-white px-3 py-2 placeholder-gray-400 text-sm outline-none" />
                <button className="px-4 py-2 bg-white/20 hover:bg-white/30 transition text-white">→</button>
              </div>
            </div>
          </div>
      ...
      {showDashboardModal && (
        ...
        >
          <div className="absolute inset-0" style={{ backdropFilter: 'blur(6px)', WebkitBackdropFilter: 'blur(6px)', background: 'rgba(0,0,0,0.35)', cursor: 'none' }} />
          ...
          >
            <h3 style={{ color: '#fff', fontWeight: 700, marginBottom: 8 }}>Choose dashboard</h3>
            <p style={{ color: '#d1d5db', fontSize: 13, marginBottom: 16 }}>Select a dashboard to continue</p>

            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <button
                onClick={() => { window.location.href = '/hacker-dash'; }}
                className="px-4 py-2 rounded-md"
                style={{ background: 'transparent', border: '1px solid rgba(255,255,255,0.12)', color: '#fff', fontWeight: 600, cursor: 'pointer' }}
              >
                Hacker Dashboard
              </button>
              ...
              <button
                ...
                className="px-4 py-2 rounded-md"
                style={{ background: 'linear-gradient(90deg, rgba(99,102,241,0.12), rgba(99,102,241,0.06))', border: '1px solid rgba(99,102,241,0.28)', color: '#e6e9ff', fontWeight: 700, cursor: 'pointer' }}
              >
                Admin Dashboard
              </button>
            </div>
            ...
            <button
              onClick={closeDashboardModal}
              aria-label="Close"
              style={{ position: 'absolute', right: 10, top: 10, background: 'transparent', border: 'none', color: '#9ca3af', cursor: 'pointer', fontSize: 16, padding: 6 }}
            >
              ✕
            </button>
          </div>
